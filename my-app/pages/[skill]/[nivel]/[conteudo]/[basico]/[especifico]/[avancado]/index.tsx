...
const Home: NextPage = () => {
  ...
  return (
      <>
        ...
        {dados?.map((item: any, index: number) => <Button key={index}><a href={`/${router.query.skill}/${router.query.nivel}/${router.query.conteudo}/${router.query.basico}/${router.query.especifico}/${router.query.avancado}/${item.replaceAll(' ','-')}`}>{item}</a></Button>)}
      </>
    )
  }
